...
import { Omit, HttpExchange, TimingEvents, ExchangeMessage } from "../../types";
...
const TimingPill = observer(
  (p: { className?: string; timingEvents: TimingEvents }) => {
    // We can't show timing info if the request is still going
    const doneTimestamp =
      p.timingEvents.responseSentTimestamp || p.timingEvents.abortedTimestamp;
    if (!doneTimestamp) return null;

    const durationMs = doneTimestamp - p.timingEvents.startTimestamp;

    return (
      <Pill className={p.className}>
        {
          durationMs < 100
            ? sigFig(durationMs, 2) + "ms" // 22.34ms
            : durationMs < 1000
            ? sigFig(durationMs, 1) + "ms" // 999.5ms
            : durationMs < 10000
            ? sigFig(durationMs / 1000, 3) + " seconds" // 3.045 seconds
...
export const ExchangePerformanceCard = inject("accountStore")(
  observer((props: ExchangePerformanceCardProps) => {
...
const CompressionDescription = observer(
  (p: {
    encodings: string[];
    ...
    return (
      <>
        {encodings.length ? (
          <>
            compressed with{" "}
            <strong>{joinAnd(encodings, ", ", " and then ")}</strong>, making it{" "}
            {compressionRatio !== undefined && decodedBodyLength ? (
              <>
                ...
                ({getReadableSize(decodedBodyLength)} to{" "}
...
const CompressionOptions = observer(
  (p: {
    encodings: string[];
    encodedBodyLength: number;
    decodedBodyLength: number | undefined;
    ...
    if (!_.isEmpty(encodingTestResults) && decodedBodyLength) {
      const realCompressionRatio = Math.round(
        100 * (1 - encodedBodyLength / decodedBodyLength)
      );
      ...
      return (
        <>
          {_(encodingTestResults)
            ...
            .map((size, encoding) => {
              const testedCompressionRatio = Math.round(
                100 * (1 - size / decodedBodyLength)
              );

              return (
                <Pill
                  key={encoding}
                  title={`${getReadableSize(
                    decodedBodyLength
                  )} would compress to ${getReadableSize(
                    size
                  )} using ${encoding}`}
                  color={
...
const CompressionPerformance = observer((p: { exchange: HttpExchange }) => {
  ...
  const clientAcceptedEncodings = asHeaderArray(
    p.exchange.request.headers["accept-encoding"]
  ).map(getEncodingName);
  ...
  return (
    <>
      {messageTypes.map((messageType) => {
        ...
        if (typeof message !== "object" || !message.body.encoded.byteLength)
          return null;
        ...
        const encodingTestResults = _.mapKeys(
          testEncodings(message),
        ...
        const betterEncodingAvailable =
          ...
          encodingTestResults[bestOtherEncoding] <
            Math.min(encodedBodySize, decodedBodySize);

        return (
          <React.Fragment key={messageType}>
            <ContentLabelBlock>
              {_.upperFirst(messageType)} Compression
            </ContentLabelBlock>
            <PerformanceExplanation>
              The {messageType} body was{" "}
...
const CachingPerformance = observer((p: { exchange: HttpExchange }) => {
  ...
  return (
    <>
      <ContentLabelBlock>Caching</ContentLabelBlock>
      {cacheDetails.map((details, i) => (
        <CollapsibleSection prefixTrigger={true} key={i}>
          <ExchangeCollapsibleSummary>
            {details.summary} {details.type === "warning" && <WarningIcon />}
