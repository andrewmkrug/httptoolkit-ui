...
const ParamMetadata = styled((p: { param: Parameter; className?: string }) => (
  <div className={p.className}>
    {[p.param.required ? "Required" : "Optional", p.param.type, p.param.in]
      .filter((x) => !!x)
      .join(" ")}{" "}
    parameter
    ...
    {p.param.enum !== undefined && p.param.enum.length > 0 ? (
      <>
        .<br />
        Valid values: {joinAnd(p.param.enum.map((v) => JSON.stringify(v)))}
      </>
    ) : (
      ""
    )}
...
const getDetailsWithWarnings = (
  details: Html | undefined,
  warnings: string[]
) =>
  [
    warnings.length &&
      warnings.map((warning, i) => <Warning key={warning}>{warning}</Warning>),
    details && <ExternalContent key="details" content={details} />
  ].filter((d) => !!d);

const ApiRequestDetails = (props: { api: ApiExchange }) => {
  const { api } = props;
  const setParameters = api.request.parameters.filter(
    (param) => !!param.value || param.required || param.defaultValue
  );

  // If that leaves us with lots of parameters then ignore the ones that
  // are just unset default values.
  const relevantParameters =
    setParameters.length > 5
      ? setParameters.filter((param) => !!param.value || param.required)
      : setParameters;

  const operationDetails = getDetailsWithWarnings(
    api.operation.description,
    api.operation.warnings
  );
    ...
    <>
      <CollapsibleSection>
        <ExchangeCollapsibleSummary>
          ...
          {!api.service.description && <DocsLink href={api.service.docsUrl} />}
        </ExchangeCollapsibleSummary>
        ...
        {api.service.description && (
          <ExchangeCollapsibleBody>
            ...
            <DocsLink href={api.service.docsUrl}>Find out more</DocsLink>
          </ExchangeCollapsibleBody>
        )}
      </CollapsibleSection>
      ...
      <CollapsibleSection>
        <ExchangeCollapsibleSummary>
          ...
          {!hasOperationDetails && <DocsLink href={api.operation.docsUrl} />}
        ...
        {hasOperationDetails && (
          <ExchangeCollapsibleBody>
            {operationDetails}
            <DocsLink href={api.operation.docsUrl}>Find out more</DocsLink>
          </ExchangeCollapsibleBody>
        )}
      </CollapsibleSection>
      ...
      {relevantParameters.length >= 1 && (
        <>
          <ContentLabelBlock>Parameters</ContentLabelBlock>
          <ParametersGrid>
...
interface ExchangeApiPlaceholderCardProps
  extends Omit<ExchangeCardProps, "children"> {
  apiName: string;
}

export const ExchangeApiPlaceholderCard = observer(
  (props: ExchangeApiPlaceholderCardProps) => {
    const { apiName } = props;
    ...
    return (
      <ExchangeCard {...props}>
        ...
        <CardSalesPitch source="api">
          <p>HTTP Toolkit Pro can show you more information about this API.</p>
          <p>
            Instantly understand this request with inline documentation for the
            endpoint, parameters, and responses.
          </p>
          <p>
            Automatically validate that this request &amp; its body match the
            API schema. See all the parameters that you're <em>not</em> sending
            too, and their default values, and get warned about deprecated or
            invalid endpoints and parameters.
          </p>
        </CardSalesPitch>
      </ExchangeCard>
    );
