...
const snippetExportOptions: _.Dictionary<SnippetOption[]> = _(
  HTTPSnippet.availableTargets()
)
...
const getExportOptionName = (option: SnippetOption) =>
  ...
  )[getExportOptionKey(option)] || option.name);
...
const ExportSnippetEditor = observer(
  (p: { exchange: HttpExchange; exportOption: SnippetOption }) => {
    ...
    return (
      <>
        <SnippetDescriptionContainer>
          <p>
            <strong>{getExportOptionName(p.exportOption)}</strong>:{" "}
            {description}
          </p>
          <p>
            <DocsLink href={link}>Find out more</DocsLink>{" "}
            <CopyButtonPill content={snippet}> Copy snippet</CopyButtonPill>
          </p>
        </SnippetDescriptionContainer>
...
const exportHar = async (exchange: HttpExchange) => {
  const harContent = JSON.stringify(await generateHar([exchange]));
  const filename = `${exchange.request.method} ${exchange.request.parsedUrl.hostname}.har`;

  saveFile(filename, "application/har+json;charset=utf-8", harContent);
};

const ExportHarPill = styled(
  observer((p: { className?: string; exchange: HttpExchange }) => (
    <PillButton
...
export class ExchangeExportCard extends React.Component<ExportCardProps> {
      ...
      <ExchangeCard {...this.props}>
        <header>
          ...
          <CollapsibleCardHeading
            onCollapseToggled={this.props.onCollapseToggled}
          >
            Export
          </CollapsibleCardHeading>
        </header>
  ...
  @observable.ref
  private snippetOption: SnippetOption = _.find(snippetExportOptions["Shell"], {
    client: "curl"
  }) as SnippetOption;
