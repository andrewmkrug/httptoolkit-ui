...
class MockPage extends React.Component<MockPageProps> {
  ...
  collapsedRulesMap = _.fromPairs(
    mapRules(
      this.props.rulesStore.draftRules,
      (rule) =>
        [
          rule.id,
  ...
  render(): JSX.Element {
    ...
    return (
      <MockPageContainer ref={this.containerRef}>
        <MockPageHeader>
          ...
          <OtherButton
            ...
            onKeyPress={clickOnEnter}
            title={"Import a saved set of rules"}
          >
          ...
          <OtherButton
            disabled={
              !isPaidUser ||
              !areSomeRulesNonDefault ||
              draftRules.items.length === 0
            }
            onClick={this.exportRules}
            onKeyPress={clickOnEnter}
            title={"Export these rules"}
          >
  ...
  readonly importRules = async () => {
    ...
    if (uploadedFile) {
      try {
        this.props.rulesStore.loadSavedRules(JSON.parse(uploadedFile));
      } catch (e) {
        reportError(e);
  ...
  readonly exportRules = async () => {
    ...
    saveFile(
      filename,
      "application/htkrules+json;charset=utf-8",
      rulesetContent
    );
  };
}
